...
const PostNotiPage = () => {
  ...
  const loadImagesByPost = async (postId) => {
    try {
      const res = await authApi().get(endpoints["post-image"](postId));
  ...
  useEffect(() => {
    const loadTag = async () => {
      try {
        ...
        console.log(p);
        // const res = ApiConfig.get(endpoints["tag"])
        ...
        loadCommentsByPostId(postId1);
        console.log(res2.data.comments);
        //   });

        loadImagesByPost(res2.data.id);
